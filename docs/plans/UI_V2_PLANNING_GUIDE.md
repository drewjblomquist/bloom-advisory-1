...
- Goal / intent:
  - Provide a sticky, always-visible navigation surface.
  - Separate header from content using a restrained liquid-glass surface while preserving the global gradient.
- Layout contract:
  - Header background: full-bleed across viewport width.
  - Header inner content: inside global container (`max-w-7xl` + standard responsive horizontal padding).
  - Left cluster: logo slot pinned far left within container.
  - Right cluster: nav slot pinned far right within container.
  - Alignment rules: left/right clusters do not drift toward center; both vertically centered.
  - Expected DOM structure (high level): global header wrapper (full-bleed) with inner container and left/right clusters.
- Typography hierarchy expectations:
  - Header text uses Navigation role tokens (defined globally); do not introduce ad-hoc sizing.
- Spacing system expectations:
  - Medium, modern SaaS header height (not oversized, not cramped).
  - Use standard responsive horizontal padding (e.g., `px-4 sm:px-6 lg:px-8`).
  - Vertical padding should be a single, intentional value from the allowed spacing scale.
- Interaction / behavior:
  - Sticky at top at all times; never scrolls away.
  - No layout shift on load or scroll; no jump when sticky engages.
  - Glass effect: semi-transparent gray tint + subtle backdrop blur; graceful fallback to tint-only if `backdrop-filter` unsupported.
  - Bottom edge transition: short vertical gradient fade from header tint to transparent (~12–24px). Optional ultra-subtle shadow only if needed, never a divider line.
  - Motion is minimal; defer nav hover/focus styling details to Nav Items spec (intended liquid-glass bubble affordance).
- Responsiveness rules (375 / 768 / 1024 / 1440):
  - Sticky behavior and layout contracts apply at all breakpoints.
  - Background remains full-bleed; inner content stays in container.
  - Logo pinned far left; nav pinned far right.
  - Mobile nav behavior is deferred; must be resolved before shipping.
- Accessibility requirements:
  - Contrast must preserve legibility of logo/nav text over glass surface.
  - Focus visibility must be clear for nav items.
  - Click/tap targets must meet minimum size expectations (use standard modern SaaS norms).
- Allowed files to touch (ownership + blast radius):
  - Single header component file + its test/verification artifact only.
  - `app/page.tsx` for composition only (no layout logic).
  - `app/globals.css` only if global glass tokens are required.
- Acceptance criteria checklist:
  - Header is sticky and always visible across scroll.
  - No CLS or visual jump on load or scroll.
  - Full-bleed header background with container-aligned inner content.
  - Logo far left and nav far right; no centering drift.
  - Glass effect is translucent and legible; fallback works without blur.
  - Bottom edge fades into background; no hard divider line.
  - Works at 375 / 768 / 1024 / 1440 and mobile Safari.
- Test / verification steps:
  - Scroll test to confirm sticky behavior and no vertical jump.
  - Resize checks at 375 / 768 / 1024 / 1440.
  - Devtools check for `backdrop-filter` fallback and computed background.
  - Verify z-index above content but below modals.
  - CLS check (no layout shift on load/hydration).
- Do not change constraints:
  - No hard divider line at header bottom.
  - No opaque fill that hides the gradient entirely.
  - No per-section header variants.
  - No layout shift on load (font swap, hydration, scroll).
  - No vertical jump when sticky engages.
  - No overflow or clipping at any breakpoint.
  - No hover effects that reduce legibility or create motion distraction.
  - No reliance on heavy libraries/effects for glass (CSS-only).
  - No z-index bugs: header above content but below modals.
  - No accidental extra top padding/margin introduced by sticky positioning.
- Decision Needed (if any):
  - Mobile nav behavior (collapse/hamburger vs alternative) must be defined before shipping.
